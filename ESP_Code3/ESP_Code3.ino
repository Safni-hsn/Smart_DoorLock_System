...
const unsigned long passwordTimeout = 10000;  // 20 seconds timeout
...
void askForPassword() {
  ...
  currentLength = 0;
  passwordEntryTime = millis();  // Start the password entry timer
}
...
void loop() {
  ...
  // Handle password entry timeout
  if (movementDetected && millis() - passwordEntryTime > passwordTimeout) {
    lcd.clear();
    lcd.print("System Ready");
    Blynk.virtualWrite(V1, "Door Status: Closed");  // Update label to Closed
    movementDetected = false;  // Reset movement detection
    doorOpen = false;  // Ensure the door is closed
    myServo.write(0);  // Close the door if open
  }
